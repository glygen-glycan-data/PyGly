...
print "acc\tid\tglytoucan\tname"
for acc in sorted(epitopes):
    print "\t".join(map(str,map(lambda h: epitopes[acc].get(h,""),("acc","id","glytoucan","name"))))
...
for acc in sorted(epitopes):
    redend = "False"
    motif = GlycoEpitopeMotif(accession=acc,name=epitopes[acc]['name'],glytoucan=epitopes[acc]['glytoucan'],redend=redend)
    if w.put(motif):
	print "Adding:",acc
    current.add(acc)
