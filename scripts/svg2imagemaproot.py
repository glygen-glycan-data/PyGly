...
This code looks for monosaccharide ids and paths in our produced svgs and creates an html image map from them, specificying the root monosaccharide.
...
for e in elements:
    if e.nodeName == 'g':
        ...
        if data_type == "Monosaccharide":
            mono_id = e.getAttribute("ID")
            id_num = mono_id.split(":")[1]
            mono_ids.append(int(id_num))
            #residue_name = e.getAttribute("data.residueName")
            #print residue_name
            #print "ok"
            ...
            #print pathname
            parsed_groups[pathname][0][0] = id_num

out = []
for g in parsed_groups:
    for path in parsed_groups[g]:
        if int(path[0]) == min(mono_ids):
            path[0] = "root"
        else:
            path[0] = "not_root"
