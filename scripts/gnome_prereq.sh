...
function getclassificationprereq () {
  get $1 | awk -F'\t' -v glycantype=$3 '$2 == "GlycanType" && $3 == glycantype && NR > 1 {print $1}' | sort -u > ./GNOme/$2
}

function getglycotreeprereq () {
  wget -O - "https://glygen.ccrc.uga.edu/sandbox/api/list.php?par=&mode=${1}" | jq -r '.[] | .glytoucan_ac' | sort -u > ./GNOme/$2
}
...
getclassificationprereq export/classification.tsv restrictions/GNOme_GlyGen_NGlycans.accessions.txt N-linked
getclassificationprereq export/classification.tsv restrictions/GNOme_GlyGen_OGlycans.accessions.txt O-linked
getglycotreeprereq "mapped_N" restrictions/GNOme_GlycoTree_NGlycans.accessions.txt
getglycotreeprereq "mapped_O" restrictions/GNOme_GlycoTree_OGlycans.accessions.txt
