...
for macc in motifgtc(accs):
    # print("Motif: ", macc, file=sys.stderr)
...
def check_idmaps(motifids,glycanids,idmaps):
    ...
    for idmapids in idmaps:
        ...
        if len(set(map(itemgetter(1),idmapids))) != len(motifids):
            bad += 4
        if len(list(map(itemgetter(0),idmapids))) != len(set(map(itemgetter(0),idmapids))):
            bad += 8
        if len(list(map(itemgetter(1),idmapids))) != len(set(map(itemgetter(1),idmapids))):
            bad += 16
...
for gtsselector in ['mapped_N','mapped_O']:
  for enzdata in gts.allglycans(gtsselector):
    ...
    # print("Structure:", gacc, file=sys.stderr)
    sandbox = dict()
    ...
    for r in enzdata["residues"]:
        ...
        for e in r['enzymes']:
            if len(e.get('rule_violations',[])) == 0:
    ...
    for macc,motif in motifs.items():
        ...
        if strict_core:
            ...
            if not check_idmaps(motifids[macc],glycanids,idmaps_withids_core):
                print("Glycan:",gacc, "Motif:", macc,file=sys.stderr)
                sys.exit(1)
