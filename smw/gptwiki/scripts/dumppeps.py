...
print "\t".join(map(str,["Accession","Peptide","Site","Glycan","TGAccession","Charge","PrecursorMZ"]))
seen = set()
w = GPTWiki()
for tgpage in w.iterpages(include_categories=['TransitionGroup']):
	...
	glycans = ",".join(map(str,glycans))
        print "\t".join(map(str,[pepid,pepseq,sites,glycans,tg.get('id'),z1,mz1]))
	seen.add((pepid,z1))
