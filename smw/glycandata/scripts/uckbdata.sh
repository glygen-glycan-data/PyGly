...
$PYTHON $PYGLY/GlycanResource/main.py GlyGenSourceFile unicarbkb_allgtc | \
...
$PYTHON $PYGLY/GlycanResource/main.py GlyGenSourceFile unicarbkb_allpubs  | \
  $PYTHON ./normalizebasecomp.py -f UCKBCOMP -p 1 -r '^comp_' -a | \
  awk '$1 ~ /comp_/ {print "-",$1,$2; next} {print $1,"-",$2}' | \
  sort -k1n,1 -k2,2 -k3n,3 | \
  awk '$2 ~ /comp_/ {print $2,$3; next} {print $1"\t"$3}' \
  > $DATA/uc2pubmed.txt 2>/dev/null
$PYTHON $PYGLY/GlycanResource/main.py GlyGenSourceFile unicarbkb_alltaxa | \
  $PYTHON ./normalizebasecomp.py -f UCKBCOMP -p 1 -r '^comp_' -a | \
  awk '$1 ~ /comp_/ {print "-",$1,$2; next} {print $1,"-",$2}' | \
  sort -k1n,1 -k2,2 -k3n,3 | \
  awk '$2 ~ /comp_/ {print $2,$3; next} {print $1"\t"$3}' \
